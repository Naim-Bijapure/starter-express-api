...
app.get("/getPool/:chainId/:walletAddress/:currentNonce/:type", function (request, response) {
    ...
    if (transactions[key]) {
        if (type === "QUEUE") {
            let filteredPool = transactions[key].filter((data) => data.nonce >= currentNonce);
        ...
        if (type === "ALL") {
            let filteredPool = transactions[key]
                .filter(
                    (item) => item.nonce < currentNonce && (item.status === "success" || item.status === "rejected")
                )
